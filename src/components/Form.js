...
export default function Form(){
    ...
    const detailsCtx=useContext(userContext);
    const {register,handleSubmit,formState:{errors}}=useForm();
    const navigate=useNavigate()
    const [show,setShow]=useState(false);

    function onSubmit(data){
        axios.post('https://localhost:44365/api/User',{
            id:0,
    ...
    return(
        ...
        <form className="col padding direct border border-secondary-subtle form-style mt-5" onSubmit={handleSubmit(onSubmit)} >
            <div className="row input-group mb-3">
                <div>
                    <div className="form-floating">
                        <input className="form-control" placeholder="שם " {...register('firstName', { required: true })} value={detailsCtx.firstName} onChange={e=>{detailsCtx.setFirstName(e.target.value)}}/>
                        <label >שם פרטי</label>
                    </div>
                </div>
            ...
            <div className="row input-group mb-3">
                <div> 
                    <div className="form-floating">
                        <input className="form-control" placeholder=" משפחה" {...register('lastName', { required: true })} value={detailsCtx.lastName} onChange={e=>{detailsCtx.setLastName(e.target.value)}}/>                                
            ...
            <div>
                ...
                <div className="col direct">            
                    ...
                    <Button onClick={()=>setShow(true)} style={{color:"black"}} variant="text" size="xlarge">+</Button>
                </div> :
                    navigate('/addChild')
                }
            </div>
            <Button style={{color:" rgb(22, 248, 233)"}} type="submit" variant="text" size="large" >שלח</Button>
        </form>
